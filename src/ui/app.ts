...
// Cartographic label culling: after the map is in the DOM, hide any lower-priority label that
// overlaps a higher-priority one (nation > capital > region > town), so nation names and place
// names don't collide. Runs post-mount because it needs getBBox (real layout; jsdom returns
// zero-size boxes which never overlap, so it's a no-op in tests).
function deconflictLabels(svg: SVGSVGElement): void {
  const tiers: [string, number][] = [
    [".nation-label", 5], [".city-capital", 4], [".region-label", 3], [".river-label", 2], [".city-town", 1],
  ];
