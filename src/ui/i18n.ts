...
export const PLAY_UI: Record<Lang, Record<string, string>> = {
  en: {
    chooseRealm: "Choose your realm", cells: "cells", cohesion: "loyalty", threats: "threats",
    ...
    tipInvest: "Instantly raise loyalty — your realm's unity — one-off, then decays normally. %p = the loyalty points added. Fortify frontier feeds your battles; shore up realm staves off civil war.",
    tipStrength: "Your territory (cell count) versus the average rival realm. Ahead = strong, behind = weak.",
    tipCohesion: "Low loyalty makes your realm weaker in battle, so you lose ground. A large realm with low loyalty can also split apart in civil war. Restore it with invest (💰) or the internal stance.",
    ...
    howto4: "Loyalty is your realm's health. Low loyalty invites civil war and erosion — restore it with invest or the internal stance.",
    solStable: "steady", solShaky: "shaky", solDanger: "critical",
    tipAggressive: "Stronger attacks · loyalty slowly decays", tipDefensive: "Stronger defense · slower expansion", tipInternal: "Loyalty recovers · expansion held back",
    adviceLowSol: "💡 Loyalty is low — invest, or switch to the internal stance to recover.",
    ...
    brLine: "front loyalty {m}% vs {t}%", brAhead: "ahead", brEven: "even", brBehind: "behind",
    brTip: "Average loyalty on each side of your border — the dominant local term in every border battle (realm-wide loyalty and size also weigh in).",
    stanceNums: "attack ×{a} · defense ×{d} · loyalty {s}%p/turn",
    ...
    goalProsper: "Prosperity — cities {c}/{max} · loyalty {ok} · streak {s}/{need}", tipGoalProsper: "Hold 6 cities with healthy loyalty for 3 consecutive turns.",
    goalEndure: "Endure — year {y}/500", tipGoalEndure: "Keep your capital until year 500.",
    fxFortify: "frontier loyalty ▲ · interior ▼", fxNoTarget: "no target to strike",
    fxOdds: "{p}% success", fxFail: "fail",
    fxTruceBreak: "breaks the truce", fxTruceGain: "truce secured",
    fxNoEffect: "no effect", fxCitywall: "loyalty ▲▲ around the city",
    fxProphecyDeal: "loyalty ▼ now · judged next decade",
    fxProphecyCond: "▲▲ if loyalty ≥50%, ▼ if below · now {p}%",
    ...
    ascBadge: "⬆ Ascension {n}", ascTip: "{n} wins on this world — every rival's loyalty regenerates {n} steps faster.",
  ...
  ko: {
    chooseRealm: "국가를 선택하세요", cells: "셀", cohesion: "민심", threats: "위협",
    ...
    tipInvest: "민심(나라의 응집력)을 즉시 올립니다 — 일회성, 이후 자연 감쇠. %p = 민심이 오르는 폭. 국경 방비 = 전투 직결, 내정 다지기 = 내전 예방.",
    tipStrength: "내 영토(셀 수)를 이웃 세력 평균과 비교합니다. 앞서면 우세, 밀리면 열세.",
    tipCohesion: "민심이 낮으면 전투에서 약해져 땅을 잃기 쉽습니다. 나라가 크고 민심까지 낮으면 내란으로 분열될 수 있습니다. 투자(💰)나 내치 태세로 회복합니다.",
    ...
    howto4: "민심은 나라의 체력입니다. 낮으면 내전과 침식이 옵니다 — 투자나 내치 태세로 회복하세요.",
    solStable: "안정", solShaky: "불안", solDanger: "위험",
    tipAggressive: "공격 강화 · 민심 서서히 감소", tipDefensive: "수비 강화 · 확장 둔화", tipInternal: "민심 회복 · 확장 억제",
    adviceLowSol: "💡 민심이 낮습니다 — 투자하거나 내치 태세로 회복하세요.",
    ...
    brLine: "국경 민심 {m}% vs 인접 적 {t}%", brAhead: "우세", brEven: "비등", brBehind: "열세",
    brTip: "국경 양쪽의 평균 민심 — 모든 국경 전투에서 가장 큰 국지 항목입니다 (전국 민심·규모도 함께 작용).",
    stanceNums: "공격 ×{a} · 수비 ×{d} · 민심 {s}%p/턴",
    border: "국경 접촉", truce: "휴전", vs: "vs",
    goals: "목표",
    goalConquest: "정복 — 라이벌 {n}국", tipGoalConquest: "모든 라이벌 국가를 무너뜨리면 정복 승리입니다.",
    goalProsper: "번영 — 도시 {c}/{max} · 민심 {ok} · 연속 {s}/{need}", tipGoalProsper: "도시 6개를 보유하고 민심을 유지한 채 3턴 연속 버티면 번영 승리입니다.",
    goalEndure: "존속 — {y}/500년", tipGoalEndure: "500년까지 수도를 지키면 존속 승리입니다.",
    fxFortify: "국경 민심 ▲ · 내지 ▼", fxNoTarget: "칠 곳 없음",
    fxOdds: "성공 {p}%", fxFail: "실패",
    fxTruceBreak: "휴전 파기", fxTruceGain: "휴전 확보",
    fxNoEffect: "변화 없음", fxCitywall: "도시 주변 민심 ▲▲",
    fxProphecyDeal: "지금 민심 ▼ · 다음 십년에 심판",
    fxProphecyCond: "민심 50% 이상이면 ▲▲, 미만이면 ▼ · 지금 {p}%",
    ...
    ascBadge: "⬆ 상승 {n}", ascTip: "이 세계에서 {n}승 — 모든 라이벌의 민심 회복이 {n}단계 강해집니다.",
...
export function playLog(lang: Lang, code: string | undefined, data: Record<string, string | number> = {}): string {
  ...
  if (lang === "ko") {
    switch (code) {
      ...
      case "invested": return `${where}에 투자: ${n}개 셀의 민심이 올랐습니다.`;
  ...
  switch (code) {
    ...
    case "invested": return `Invested in ${where}: loyalty raised on ${n} cells.`;
...
export function playDilemma(lang: Lang, code: string, data: Record<string, string | number> = {}): { title: string; a: string; b: string } {
  ...
  if (lang === "ko") {
    switch (code) {
      case "unrest": return { title: "제후들의 불만이 끓어오릅니다.", a: "변경 영지를 양보한다 (영토 −, 민심 +)", b: "강경 진압한다 (도박)" };
      case "raiders": return { title: "국경에 습격이 잇따릅니다.", a: "국경을 요새화한다 (국경 민심 +)", b: "보복 원정을 보낸다 (무료 공격)" };
      case "warweary": return { title: "잇단 전쟁에 백성이 지쳐갑니다.", a: "징집을 강화한다 (국경 ▲▲, 내지 ▼)", b: "최대 위협국과 화의를 모색한다 (20년 휴전, 민심 소폭 ▼)" };
      case "boomtown": return { title: "건설한 도시가 크게 성장했습니다.", a: "시장 특허를 내린다 (전국 민심 ▲)", b: "성벽을 증축한다 (도시 주변 ▲▲)" };
      case "prosperity": return { title: "나라에 풍년이 들었습니다.", a: "대축제를 연다 (전국 민심 +)", b: "변경 개척에 투자한다 (국경 민심 +)" };
      ...
      case "prophecy2": return { title: "예언의 시간이 왔습니다 — 나라의 민심이 심판대에 오릅니다.", a: "성취를 선포한다 (민심 ≥50%: ▲▲ / 미만: ▼)", b: "조용히 묻는다 (변화 없음)" };
      case "hegemon1": return { title: `${name}이(가) 패권국으로 부상했습니다. 그 그림자가 국경에 드리웁니다.`, a: "측면을 규합한다 (이웃과 휴전)", b: "군비를 증강한다 (국경 ▲▲, 내지 ▼)" };
      case "hegemon2": return { title: `${name}의 최후통첩 — 조공이냐, 전쟁이냐.`, a: "조공을 바친다 (민심 ▼▼, 30년 휴전)", b: "항전을 결의한다 (민심 ▲, 결전으로)" };
      case "hegemon3": return { title: `결전의 날 — ${name}의 대군이 국경에 집결했습니다.`, a: "결전에 나선다 (도박 — 승률은 민심이 정한다)", b: "무릎 꿇는다 (민심 ▼▼, 30년 휴전)" };
    }
  }
  switch (code) {
    case "unrest": return { title: "The lords seethe with discontent.", a: "Concede border fiefs (lose land, regain loyalty)", b: "Crush them (a gamble)" };
    case "raiders": return { title: "Raiders harry the frontier.", a: "Fortify the border (frontier loyalty +)", b: "Send a punitive raid (free strike)" };
    case "warweary": return { title: "The realm wearies of endless war.", a: "Raise the levies (border ▲▲, interior ▼)", b: "Sue for terms with the greatest threat (20y truce, loyalty slightly ▼)" };
    case "boomtown": return { title: "Your founded city booms.", a: "Charter the market (realm loyalty ▲)", b: "Raise the walls (▲▲ around the city)" };
    case "prosperity": return { title: "The realm prospers.", a: "Hold a great festival (realm loyalty +)", b: "Fund the frontier (border loyalty +)" };
    ...
    case "prophecy2": return { title: "The prophecy's hour has come — the realm's loyalty is judged.", a: "Proclaim the fulfilment (loyalty ≥50%: ▲▲ / below: ▼)", b: "Bury it quietly (no effect)" };
    ...
    case "hegemon2": return { title: `${name}'s ultimatum — tribute, or war.`, a: "Pay tribute (loyalty ▼▼, 30y truce)", b: "Defy them (loyalty ▲, to the reckoning)" };
    case "hegemon3": return { title: `The reckoning — ${name}'s host masses on your border.`, a: "Give battle (a gamble — loyalty sets the odds)", b: "Kneel (loyalty ▼▼, 30y truce)" };
  }
