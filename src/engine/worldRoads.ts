 ...
 */
export function roadsBetweenTowns(mesh: Mesh, heights: ArrayLike<number>, terrain: ArrayLike<number>, towns: { cell: number }[]): RoadOut[][] {
  ...
  return out.map((roads) => roads.sort((x, y) => x.to - y.to));
  ...
  function leaving(a: number, b: number): number {
    ...
    path.reverse();                               // path[0] is a's own cell
